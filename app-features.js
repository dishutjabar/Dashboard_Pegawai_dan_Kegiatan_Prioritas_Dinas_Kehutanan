...
function resetFilter() {
  FILTER = { cdk: [], pegawaiUnit: [], kab: [], status: [], kawasan: [], penyuluh: [], jabatan: [], nama_pegawai: [] };
  ['f_cdk','f_pegawai','f_kab','f_status','f_kawasan','f_penyuluh', 'f_jabatan', 'f_nama_pegawai'].forEach(function(id) {
    try { 
...
function passFilter(r, type) {
  ...
  if (type === 'pjl' && FILTER.penyuluh && FILTER.penyuluh.length > 0 && !FILTER.penyuluh.includes(String(r['Penyuluh Kehutanan'] || '').trim())) return false;
  return true;
}
...
function fillDropdown() {
  var S = { cdk: new Set(), unit: new Set(), kab: new Set(), status: new Set(), kawasan: new Set(), penyuluh: new Set(), jabatan: new Set(), nama_pegawai: new Set() };
  ['pjl','persemaian','pegawai','jumat'].forEach(t => {
    DATA[t].forEach(r => {
      if(!r) return;
      var c = getCDK(r['Unit Kerja']); if(c) S.cdk.add(c);
      if(r._kab) S.kab.add(r._kab);
      if(t==='pjl') { var kw=r['Kawasan Leuweung/ Gunung']; if(kw) S.kawasan.add(kw.trim()); var py=r['Penyuluh Kehutanan']; if(py) S.penyuluh.add(py.trim()); }
  ...
  pop('f_status', S.status, 'Semua Status'); pop('f_kawasan', S.kawasan, 'Semua Kawasan'); pop('f_penyuluh', S.penyuluh, 'Semua Penyuluh');
...
function doRender() {
  ...
  if (isFilterActive && pegNames.length > 0 && jumNames.length > 0 && typeof turf !== 'undefined') {
    var popupHTML = '<div style="font-family:Inter; font-size:12px;"><b>Area Pegawai & Jum\'at Menanam</b><br>';
    popupHTML += '<div style="max-height:150px; overflow-y:auto; margin-top:5px; border-top:1px solid #ddd; padding-top:5px;">';
    popupHTML += '<b style="color:#8e24aa;">Lokasi Jum\'at Menanam:</b><br>' + [...new Set(jumNames)].join('<br>') + '<br><br>';
    popupHTML += '<b style="color:#fb8c00;">Pegawai:</b><br>' + [...new Set(pegNames)].join('<br>') + '</div></div>';
